    ...
    methods: {
      ...
      fetchData() {
        return axios.get("https://blockchainbird.com/t/cardgame-resources/data/data-csv-cors.php")
          .then(response => {
            // console.log('d3.csvParse(response.data): ', d3.csvParse(response.data.length));
            // this.createCategoryNames(response.data);
      ...
      createCategoryNames(dataCardsAll) {
        ...
        // categoryNames.filter(onlyUnique)
        console.log('categoryNames.filter(onlyUnique): ', categoryNames.filter(onlyUnique));
