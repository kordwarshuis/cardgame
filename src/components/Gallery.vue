<template>
<!-- /*! https://codepen.io/hailedev/pen/oNjmKgM */ -->
<div class="gallery" :class="{ 'gallery-display': show }" @mouseleave="hideGallery">
    <div>
        <div v-for="(imageRow, i) in imageRows" class="gallery__row">
            <div v-for="(image, j) in imageRow" class="gallery__row__image" :style="{ width: `${100 / columns}%` }">
                <div :class="[
              'postcard',
              { 'postcard-transition': transition === image }
            ]" :style="getPostcardStyle(i, j, image)" @click="(e) => selectImage(i, j, image)" :ref="`pc_${i}${j}`">
